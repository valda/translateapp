<script lang="ts">
  ...
  import type { HistoryItem, SettingsResponse, DetectResponse, ModelsResponse } from '$lib/types';
  ...
  <header class="mb-10 text-center">
    <div class="flex items-center justify-center gap-3">
      ...
      <div class="flex items-center gap-2">
        <!-- 接続ステータス -->
        <span
          class="inline-block h-2.5 w-2.5 rounded-full {connectionOk === true
            ? 'bg-green-500'
            : connectionOk === false
              ? 'bg-red-500'
              : 'bg-stone-300'}"
          title={connectionOk === true
            ? 'Ollama接続中'
            : connectionOk === false
              ? 'Ollama未接続'
              : '確認中...'}
      ...
      >
        ...
        <div class="mb-5 flex items-center gap-2">
          ...
          <span class="text-ink-muted text-sm">
            {connectionOk === true ? '接続中' : connectionOk === false ? '未接続' : '確認中...'}
          </span>
        </div>
